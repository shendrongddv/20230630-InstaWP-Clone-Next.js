...
export const CardAwwards = () => {
  ...
  return (
    <ul className="mt-4 grid gap-1 md:grid-cols-3">
      {items?.map((item) => (
        <li
          key={item.id}
          className="flex items-center gap-4 rounded-xl px-4 py-4 transition-colors duration-300 hover:bg-slate-50 md:rounded-3xl md:px-8"
        >
          <CardAwward
            title={item.title}
